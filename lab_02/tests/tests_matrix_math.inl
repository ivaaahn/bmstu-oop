...
TEST(MatrixMathTests, NonSquare)
{
    ...
    EXPECT_THROW(a.determinant(), InvalidDimensions);
}
...
TEST(MatrixMathTests, Transpose0x0)
{
    ...
    EXPECT_EQ(a.transpose(), reference);
}
...
TEST(MatrixMathTests, InverseNonSquare)
{
    ...
    EXPECT_THROW(a.inverse(), InvalidDimensions);
}
...
TEST(MatrixMathTests, InverseSingular)
{
    ...
    EXPECT_THROW(a.inverse(), SingularMatrix);
}

TEST(MatrixMathTests, Minor)
{
    Matrix<double> a = { {1, 2}, {3, 4} };
    EXPECT_EQ(a.minor(0,0), 4);
}

TEST(MatrixMathTests, MinorNonSquare)
{
    Matrix<double> a = { {1, 2, 3}, {4, 5, 6} };
    EXPECT_THROW(a.minor(0,0), InvalidDimensions);
}
